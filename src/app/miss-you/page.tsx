    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="text-center mb-10 animate-slide-up">
          <h1 className="font-script text-4xl text-rose-500 mb-2">想你了</h1>
          <p className="text-rose-400 text-lg font-serif">每一下点击都是满满的思念</p>
        </div>

        <div className="grid grid-cols-2 gap-6 mb-10">
          {["A", "B"].map((who) => (
            <motion.button key={who} whileTap={{ scale: 0.95 }}
              ...
              <span className="text-xs text-rose-300">次想念</span>
                  ...
                  <>
                    {[...Array(8)].map((_, i) => (
                      <motion.div key={i}
                        ...
                        animate={{ opacity: 0, scale: 1, x: (Math.random() - 0.5) * 200, y: (Math.random() - 0.5) * 200 }}
          ...
          {stats.total > 0 && (
            <div className="mt-4">
              <div className="h-2 bg-rose-100 rounded-full overflow-hidden flex">
                ...
                <div className="bg-rose-300 transition-all duration-500"
        ...
        <div className="kitty-card p-6">
          <h2 className="font-serif text-lg font-bold text-rose-500 mb-4">最近的想念</h2>
          {records.length === 0 ? (
            <p className="text-center text-rose-300 py-4">还没有记录，快去按一下吧~</p>
          ) : (
                ...
                <motion.div key={record.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-rose-600">
                      <span className="font-bold">{record.fromWho === "A" ? "航林" : "佳钰"}</span> 想 TA 了
                    </p>
                    {record.message && (
                      <p className="text-xs text-rose-400 mt-0.5 italic">&ldquo;{record.message}&rdquo;</p>
                    )}
                  </div>
